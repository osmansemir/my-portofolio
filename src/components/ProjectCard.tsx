...
export default function ProjectCard({
  ...
  return (
    <div
      className={cn(
        " bg-creamy rounded-2xl flex flex-col overflow-hidden border hover:shadow-md",
        className,
      )}
    >
      ...
      <div className="flex flex-col px-3 ">
        <div className="space-y-2">
          <h3 className="font-bold tracking-tight dark:text-background mt-4 text-lg">
            {project.title}
          </h3>
          <div className=" max-w-full text-pretty text-base/5 pb-2 overflow-hidden font-sans min-h-5 text-secondary/70">
      ...
      <div className="text-pretty font-sans text-sm text-muted-foreground mt-auto flex flex-col px-3">
        <div className="mt-2 flex flex-wrap gap-1">
          {project.tech?.map((item) => (
            <div
              key={item}
              className=" rounded-full bg-secondary text-background dark:text-foreground px-2 py-1 text-xs/tight"
            >
              {item}
            </div>
          ))}
